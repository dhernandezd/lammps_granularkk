...
void PairGranHookeHistoryKokkos<DeviceType>::operator()(TagPairGranHookeHistoryCompute<NEIGHFLAG,NEWTON_PAIR,VFLAG,SHEARUPDATE>, const int ii, EV_FLOAT &ev) const {
  ...
  //const int nFreq = 125000; //print frequency steps
  //std::ofstream force_file;
  //if (update->ntimestep % nFreq == 0){
  //  
  //  std::ostringstream filename;
  //  filename << "rotating_drum." << int(update->ntimestep / nFreq == 0) << ".forces";
  //  force_file.open(filename.str());
  //}
  ...
  for (int jj = 0; jj < jnum; jj++) {
    ...
    //added by DHD to swap Hooke with Hertz
    F_FLOAT polyhertz = sqrt((radsum-r)*irad*jrad / radsum);
    ccel *= polyhertz;

    //F_FLOAT ndamp = 0.0;
    //if (SHEARUPDATE) ndamp = damp * polyhertz;
    ...
    //F_FLOAT tdamp = meff*gammat*sqrt(vtr1*vtr1 + vtr2*vtr2 + vtr3*vtr3); 

    //if (tdamp > meff*gammat*sqrt(vtr1*vtr1 + vtr2*vtr2 + vtr3*vtr3)) tdamp = fn;
    ...
    //if (update->ntimestep % nFreq == 0) {
    //    force_file << i << j << fx << fy << fz << std::endl;
    //  }
    //if (update->ntimestep % nFreq == 0) 
    //printf("%d %d %f %f %f/n", i, j, fx, fy, fz);
