...
void FixWallGranKokkos<DeviceType>::operator()(TagFixWallGranHookeHistory<WallStyle>, const int &i) const
{
  ...
  if (mask[i] & groupbit) {
    ...
    if (rsq > radius*radius) {
      ...
      //added by DHD to account for drum rotation
      double vr1 = v(i,0) + vshear*x(i,1);
      double vr2 = v(i,1) - vshear*x(i,0);
      double vr3 = v(i,2);
      ...
      //added by DHD to swap Hook with Hertz
      double polyhertz = sqrt((radius-r)*radius);
      ccel *= polyhertz;
